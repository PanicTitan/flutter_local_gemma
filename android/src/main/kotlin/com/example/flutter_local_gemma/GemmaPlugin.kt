...
class GemmaPlugin : FlutterPlugin, MethodChannel.MethodCallHandler, EventChannel.StreamHandler {
          ...
          private lateinit var jsSkillChannel:               MethodChannel
          private lateinit var jsSkillExecutor:           JsSkillExecutor
          private var agentToolSet:                                         GemmaAgentToolSet? = null
          ...
          override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
                    ...
                    jsSkillExecutor = JsSkillExecutor(appContext)
                    agentToolSet = GemmaAgentToolSet(jsSkillExecutor, methodChannel)
                    jsSkillChannel = MethodChannel(binding.binaryMessenger, "flutter_local_gemma/js_skill")
                    jsSkillChannel.setMethodCallHandler { call, result ->
                              when (call.method) {
                                        "registerSkills" -> {
                                                  val skills = call.argument<List<Map<String, Any>>>("skills") ?: emptyList()
                                                  agentToolSet?.registeredSkills = skills.mapNotNull {
                                                            val name = it["name"] as? String ?: return@mapNotNull null
                                                            val desc = it["description"] as? String ?: return@mapNotNull null
                                                            val inst = it["instructions"] as? String ?: return@mapNotNull null
                                                            val scriptHtml = it["scriptHtml"] as? String
                                                            val timeoutMs = (it["timeoutMs"] as? Number)?.toLong() ?: 30_000L
                                                            GemmaAgentToolSet.SkillDef(name, desc, inst, scriptHtml, if (timeoutMs > 0) timeoutMs else Long.MAX_VALUE)
                                                  }
                                                  result.success(null)
                                        }
                                        "executeJsSkill" -> {
                                                  val skillName = call.argument<String>("skillName") ?: ""
                                                  val scriptName = call.argument<String>("scriptName") ?: "index.html"
                                                  val argsJson = call.argument<String>("argsJson") ?: "{}"
                                                  val secret = call.argument<String>("secret") ?: ""
                                                  val timeoutMs = call.argument<Int>("timeoutMs")?.toLong() ?: 30_000L
                                                  
                                                  val skill = agentToolSet?.registeredSkills?.find { it.name == skillName }
                                                  if (skill == null || skill.scriptHtml == null) {
                                                            result.error("NOT_FOUND", "Skill not found or has no HTML", null)
                                                            return@setMethodCallHandler
                                                  }
                                                  
                                                  ioScope.launch {
                                                            try {
                                                                      val res = jsSkillExecutor.execute(
                                                                                skillName = skillName,
                                                                                scriptName = scriptName,
                                                                                scriptHtml = skill.scriptHtml,
                                                                                argsJson = argsJson,
                                                                                secret = secret,
                                                                                timeoutMs = if (timeoutMs > 0) timeoutMs else Long.MAX_VALUE
                                                                      )
          ...
          override fun onDetachedFromEngine(binding: FlutterPlugin.FlutterPluginBinding) {
                    ...
                    jsSkillChannel.setMethodCallHandler(null)
                    jsSkillExecutor.dispose()
                    copyProgressSink = null
          ...
          override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
                    when (call.method) {
                              ...
                              "createSession" -> {
                                        ...
                                        val nativeToolCalling = call.argument<Boolean>("nativeToolCalling") ?: false
                                        autoStopEnabled     = call.argument<Boolean>("autoStopEnabled") ?: true
                                        maxRepetitions      = call.argument<Int>("maxRepetitions") ?: 5
                                        ioScope.launch {
                                                  safeResultCall(result) {
                                                            openConversation(temperature, topP, topK, systemPrompt, enableThinking, nativeToolCalling)
                                                            null
                                                  }
                                        }
                              }
          ...
          private fun loadEngine(path: String, maxTokens: Int, backendIdx: Int, audio: Boolean) {
                    ...
                    try {
                              ...
                              catch (e: UnsatisfiedLinkError) { Log.w(TAG, "JNI auto-loaded by AAR: ") }

                              var speculativeSupported = false
                              com.google.ai.edge.litertlm.Capabilities(resolvedPath).use {
                                        speculativeSupported = it.hasSpeculativeDecodingSupport()
                              }
          ...
          private var isThinkingEnabled = false

          @OptIn(ExperimentalApi::class)
          private fun openConversation(temp: Float, topP: Float, topK: Int, systemPrompt: String?, enableThinking: Boolean, nativeToolCalling: Boolean) {
                    val eng = engine ?: throw IllegalStateException("Engine not initialised. Call createModel first.")
                    closeConversation()
                    
                    this.isThinkingEnabled = enableThinking
                    ...
                    val sysMsg: Contents? = systemPrompt
                              ?.takeIf { it.isNotBlank() }
                              ?.let { Contents.of(listOf(Content.Text(it))) }
                              
                    // Dart handles all tool intercepts via the unified stream loop.
                    // Passing tools to the C++ engine enables constrained decoding, which
                    // forces the model to use the multiplexer (execute_skill) and conflicts
                    // with our custom prompt declarations.
                    ExperimentalFlags.enableConversationConstrainedDecoding = false
                    conversation = eng.createConversation(ConversationConfig(
                              systemInstruction = sysMsg
                    ))
                    Log.i(TAG, "Session opened — temp=$temp topP=$topP topK=$topK toolsEnabled=false enableThinking=$enableThinking")
          }
          ...
          private fun startStreamingGeneration() {
                    ...
                    generationJob = ioScope.launch {
                              try {
                                        val extraContext = mapOf("enable_thinking" to isThinkingEnabled)
                                        conv.sendMessageAsync(Message.of(parts), extraContext = extraContext)
                                                  .catch { e ->
                                                  ...
                                                  .onCompletion { 
                                                            if (wasThinking) {
                                                                      withContext(Dispatchers.Main) { sendStreamPartial("<channel|>") }
                                                            }
                                                            withContext(Dispatchers.Main) { sendStreamDone() } 
                                                  }
                                                  .collect { partial ->
                                                            val textToken = partial.toString()
                                                            var thoughtToken = ""

                                                            try {
                                                                      val channelsField = partial::class.java.getDeclaredField("channels")
                                                                      channelsField.isAccessible = true
                                                                      val channels = channelsField.get(partial) as? Map<*, *>
                                                                      if (channels != null && channels.containsKey("thought")) {
                                                                                thoughtToken = channels["thought"]?.toString() ?: ""
                                                                      }
                                                            } catch (e: Exception) {
                                                            ...
                                                            if (thoughtToken.isNotEmpty()) {
                                                                      if (!wasThinking) {
                                                                                tokenBuilder.append("<|channel>thought\n")
                                                                                wasThinking = true
                                                                      }
                                                                      tokenBuilder.append(thoughtToken)
                                                            }

                                                            if (textToken.isNotEmpty()) {
                                                                      if (wasThinking) {
                                                                                tokenBuilder.append("<channel|>")
                                                                                wasThinking = false
                                                                      }
                                                                      tokenBuilder.append(textToken)
                                                            }
          ...
          private fun generateBlocking(): String {
                    ...
                    val extraContext = mapOf("enable_thinking" to isThinkingEnabled)
                    return conv.sendMessage(Message.of(parts), extraContext = extraContext).toString()
          }
